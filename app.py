...
<head>
    ...
    <style>
        ...
        input { 
            width: 100%; padding: 15px; border: 1px solid #ccc; border-radius: 0; 
            font-size: 14px; font-family: 'Inter', sans-serif; box-sizing: border-box; background: #fff;
        }
        input:focus { outline: 2px solid #000; border-color: #000; }

        button { 
            padding: 15px 30px; cursor: pointer; background: #000; color: #fff; border: 1px solid #000; 
            border-radius: 0; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; font-size: 12px; transition: all 0.2s;
        }
        ...
        .card { 
            background: #fff; border: 1px solid #e5e5e5; border-radius: 0; display: flex; flex-direction: column; 
...
# --- FUNÇÃO GEOCODING CORRIGIDA (ADMIN) ---
def geocode_address(address_str):
    try:
        # User-Agent ÚNICO e timeout aumentado para evitar bloqueios
        geolocator = Nominatim(user_agent="soul_cycles_finder_system_v10", timeout=10)
        location = geolocator.geocode(address_str)
        if location:
            print(f"Sucesso GPS: {address_str} -> {location.latitude}, {location.longitude}")
            return location.latitude, location.longitude
        else:
            print(f"Falha GPS: Endereço não encontrado: {address_str}")
    except Exception as e:
        print(f"Erro Crítico GPS: {e}")
    return None, None
...
def api_lojas():
    ...
    if cep_busca:
        try:
            # 1. Busca endereço no ViaCEP (Brasil Oficial)
...
def force_geo(id):
    ...
    if l:
        ...
        else: msg = "Endereço não encontrado pelo GPS. Verifique a escrita."
        conn.commit()
    conn.close()
